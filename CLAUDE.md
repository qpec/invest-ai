...
**Architecture revision 2026-08-03 (owner-directed): the Scout is the starting engine, and the "no LLM in the scheduled runtime" lock is lifted** — journaled in `stock-scout/docs/THESIS-DESIGN.md` §1. The scout's top 1% feeds a thesis builder (`stock-scout/thesis.py` + stdlib Claude client `stock-scout/llm.py`, model claude-opus-5, server-side web search + strict tools; edgartools (MIT) as desk-side filings-text grounding behind a guarded import); output is a DRAFT thesis (FR2 schema, machine-validatable triggers, no conviction fields — FR9) plus executive summary and extensive report; the owner ratifies at the Gate (`thesis.py ratify`) and only then does the weekly monitor (`stock-scout/monitor.py`) validate: metric triggers mechanically off `scoring.evaluate` with persistence streaks, event/narrative triggers via LLM verdicts (narrative can only send to review; event breaks need high confidence; broken is sticky until the desk acts; no API key ⇒ UNCHECKED, loudly). FR11 unchanged: never executes trades.
