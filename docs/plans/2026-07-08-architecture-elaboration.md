...
**Parked, unchanged:** technology/runtime (the next design step); eToro API verification (the E.1 contract makes it a swap-in); SEC EDGAR fallback; TradingView ToS posture (accepted, human-run only). The review's undefended/refuted proposals were **not adopted** (owner decision 2026-07-08) — the design stands as amended above.
