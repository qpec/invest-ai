...
- **S0 — 7-day daily cadence** (§1.4): the daily timer fires 7 days/week, full G.1 letter per US market day, two-line pulse Sun/Mon. This amends the owner-approved loop spine ("once per market day") and is therefore the owner's to ratify, not the panel's (review governance fix — the reasoning is the dead-man contract §11.4; the cost is two extra short messages every weekend). Recommended **accept**; until ratified, the implementation plan must not treat it as decided.
- **S1 — certifi MPL-2.0 exception** (§2.2): unavoidable via the locked yfinance tree; recommended **accept**, signed once at bootstrap, covering the venv (first entry of the named-exception list).
...
- **S3 — second-disk backup target** (§11.6): on by default; a physical second disk (mounted `/mnt/agentcy-backup`) must be designated at install.
